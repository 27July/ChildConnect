...
export default function ChatScreen() {
  ...
  const translateText = async (original, nextLang, key) => {
    ...
    try {
      ...
      setTranslations((prev) => ({
        ...prev,
        [key]: { text: data.translatedText || "⚠️ Failed", lang: nextLang },
      }));
    } catch (err) {
  ...
  const renderMessage = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity onPress={() => handleCycleTranslation(item, key)} className={`flex-row mb-3 ${isMe ? "justify-end" : "justify-start"}`}>
        ...
        <View style={{ maxWidth: "78%", backgroundColor: isMe ? "#C6E3DE" : "#FFFFFF", paddingVertical: 6, paddingHorizontal: 12, borderRadius: 20, flexShrink: 1 }}>
          ...
          {item.message && <Text className="text-[15px] text-gray-800 leading-snug">{displayText}</Text>}
